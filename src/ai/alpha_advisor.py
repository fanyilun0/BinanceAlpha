...
2. 价格稳定性（重要指标，权重35%）：
   - 24h、7d和30d价格变化：波动不应过大，尤其与大盘相比
   - 价格异常监测：无人为拉盘或砸盘行为
   - 价格健康状况：无漏洞操纵迹象，无突然巨幅波动
   - 随大盘抗压能力：市场下跌时表现出的抵抗力和恢复能力
   币安强调价格必须保持相对稳定，无"重大暴跌或哄抬价格行为"。
...
请基于币安官方公告的四大关键考量因素，对{f"{platform}平台上的" if platform else ""}币安Alpha项目进行全面权重评估。

推荐3-5个最符合现货上币要求的代币，对每个代币进行以下分析：
   1. 基本信息：以表格呈现代币名称、代码、交易量、市值、FDV和总上现货评分(1-10分)
   2. 四大因素加权分析：
      - 交易量表现(45%)：详细分析24h/7d/30d交易量表现及VOL/MC比率
      - 价格稳定性(35%)：详细分析价格波动情况、稳定性趋势及抗压表现
      - 监管合规性(10%)：项目合规状况及团队背景评估
      - 代币分配与解锁(10%)：MC/FDV比例解读及代币分配合理性
   3. 总体评估：项目核心优势、主要风险点和现货上币前的改进建议
   4. 最终加权得分：按币安权重计算的总得分及上币可能性评估
